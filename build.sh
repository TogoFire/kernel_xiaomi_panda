...
if [[ "$OS" == *"SUSE"* ]] || [[ "$OS" == *"Regata"* ]] || [[ "$OS" == *"Fedora"* ]] || [[ "$OS" == *"Nobara"* ]] || [[ "$OS" == *"Ultramarine"* ]] || [[ "$OS" == *"Rocky"* ]] || [[ "$OS" == *"Arch"* ]] || [[ "$OS" == *"Manjaro"* ]] || [[ "$OS" == *"Endeavour"* ]] || [[ "$OS" == *"Garuda"* ]]; then
    ...
    if [[ -z $(ls -A | grep -E 'clang-neutron|clang-weebx|clang-greenforce|clang-trb') ]]; then
        echo -e "$yellow No existing Clang versions found. Proceeding to download... $white"
        read -p "Which version of Clang would you like to use? (1) clang-neutron, (2) clang-weebx, (3) clang-greenforce or (4) clang-trb: " VERSION_CHOICE
